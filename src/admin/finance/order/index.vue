  ...
  <div class="tabs">
    ...
    <div class="tab-content">
      <el-table
        ...
        v-loading="loading"
      >
        <el-table-column prop="orderNo" label="订单编号" width="190px" show-overflow-tooltip></el-table-column>
        <el-table-column label="订单明细" show-overflow-tooltip>
          <template slot-scope="scope">
            <p
              v-for="item in scope.row.orderDetails"
              :key="item.id"
            >{{item.targetTypeString}}：{{item.goodsName}}</p>
          </template>
        </el-table-column>
        <el-table-column prop="user.username" width="150" label="用户名" show-overflow-tooltip></el-table-column>
        <el-table-column prop="price" label="金额" width="120" sortable show-overflow-tooltip>
          <template slot-scope="scope">{{scope.row.price}}元</template>
        </el-table-column>
        <el-table-column prop="creatime" label="提交时间" width="200" sortable show-overflow-tooltip></el-table-column>
    ...
    <el-dialog title="订单详情" :visible.sync="dialogOrder">
      ...
      <el-table :data="orderInfo.orderDetails" stripe style="width: 100%">
        <el-table-column prop="targetTypeString" label="商品类型" width="180"></el-table-column>
