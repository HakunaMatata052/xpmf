		...
		methods: {
			pay(type) {
				...
				that.post_json(that.$store.state.api + 'recharge/', {
					...
					if(type == 20) {
						that.$alert('<iframe style="width:100%;height:241px;margin:auto" scrolling="no" src="'+that.$store.state.wxpay+'/example/NativePayPage.aspx?no=' + data.no + '&price=' + data.price + '&description=' + data.description + '&attach=recharge" frameborder="0"></iframe>', '扫码支付', {
