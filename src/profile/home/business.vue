...
export default {
  ...
  methods: {
    ...
    bindDomainFn() {
      var that = this;
      if (that.domainForm.domain == "" || that.domainForm.domain.trim().length <= 0 || that.domainForm.domain == undefined) {
        that.$message({ type: "error", message: "域名不能为空" });
        return;
      }
