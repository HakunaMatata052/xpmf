<template>
  <div id="site">
    ...
    >
      <div class="site-list-left">
        <div
          ...
          @mouseout="siteImgShowId=null"
          @click="setUploadId(index,item.siteId,item.siteName)"
        >
          <el-upload
            :ref="index"
            class="site-img-uploads"
            name="upload"
            :action="$store.state.api+'UserSite/thumbnail'"
            :headers="headers"
        ...
        <div class="site-info">
          <h3>{{item.siteName}}</h3>
          <p><a
              :href="'http://'+item.userSiteBindDomains[0].domain"
              target="_blank"
              v-if="item.userSiteBindDomains.length!=0"
            >http://{{item.userSiteBindDomains[0].domain}}</a></p>
          <div class="btn-group">
            <el-button
              size="small"
              type="success"
              @click="bindDomainFn(item.siteId)"
            >绑定域名</el-button>
            <el-button
              size="small"
              type="primary"
            ...
            <el-button
              size="small"
              @click="bindFn('host',item.siteId)"
            >绑定空间</el-button>
            <el-button
              size="small"
              @click="bindFn('wuyou',item.siteId)"
            >绑定无忧</el-button>
            <el-button
              size="small"
              @click="bindFn('service',item.siteId)"
            >绑定服务</el-button>
          </div>
        </div>
      </div>
      <div class="site-list-right">
        <div class="btn-group">
          <el-button
            size="small"
            @click="jump_router('/process')"
          >工单</el-button>
          <el-button
            size="small"
            @click="ftpFn(item.siteId)"
          >FTP</el-button>
          <el-button
            size="small"
            type="danger"
            @click="delSiteFn(item.siteId)"
          >删除</el-button>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        <el-form-item>
          <el-button
            type="primary"
            @click="submitCreateSite('createSiteForm')"
          >立即创建</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
    ...
    <el-dialog
      title="绑定"
      :visible.sync="$store.state.dialogbind"
      @closed="closeddialogbind"
    >
    ...
    <el-dialog
      title="绑定域名"
      :visible.sync="dialogBindDomain"
    >
      <el-form
        :model="bindDomainForm"
        :rules="bindDomainRules"
      >
        ...
        >
          <template slot-scope="scope">
            <el-button
              type="danger"
              icon="el-icon-delete"
              @click="delBindDomain(scope.row.id)"
              circle
            ></el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    bind() {
      var that = this;
      if (that.bindDomainForm.domain.trim().length == 0) {
        that.$message({
          type: 'error',
          message: '域名不能为空'
        });
